...
이 목록은 실제 API 응답(2026-08-19, HIGHWAY_API_KEY로 조회) 58건과,
각 노선이 어느 시/군을 지나는지 위키백과·나무위키 검색으로 확인한
경계 나들목을 더해서 만들었습니다 — 예를 들어 같은 경부선 응답 안에
"금토JC-양재IC"(수도권, 성남~서초)와 "신탄진IC-회덕JC"(대전)가 함께
있었고, 경부선은 안성IC(경기)까지, 천안IC(충남)부터 수도권 밖으로
확인했습니다. 다만 전국 모든 나들목을 다 검증한 목록은 아니라서,
...
NON_CAPITAL_REGION_SEGMENT_KEYWORDS = [
    ...
    # 중부내륙선: "중부"라는 이름과 달리 여주(경기) 다음부터 충주·괴산은
    # 물론 경상북도(문경·상주·구미)·대구·경상남도(창녕)까지 훨씬 멀리
    # 뻗어 있음 — 실제로 "감곡IC-여주JC"(음성), "문경새재IC-연풍IC"(문경),
    # "선산IC-낙동JC"(구미)가 알림에 샌 적이 있어 확인 후 추가
    "감곡", "문경", "새재", "연풍", "상주", "선산", "낙동", "현풍", "창녕", "남지",
    # 영동선: 용인/양지를 지나 원주부터 수도권 밖(강원)
    "원주", "문막", "만종", "횡성", "새말", "둔내", "면온", "평창", "진부", "강릉", "대관령",
    # 서해안선: 화성/평택을 지나 당진부터 수도권 밖(충남 이남)
    "당진", "서산", "홍성", "보령", "서천", "군산", "부안", "고창", "무안", "목포",
    # 서울양양선: 가평을 지나 춘천(강원)부터 수도권 밖 — 경유지가
    # "가평→춘천→홍천→인제→양양" 순서라 춘천을 놓치면 안 됨
    "춘천", "홍천", "인제", "양양", "속초",
]
