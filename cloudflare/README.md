...
> 이 문서의 원격 D1 변경, secret 등록, Worker 배포·재배포, Discord 명령 등록은 절차 참고용입니다. 에이전트나 작업자는 각 원격 변경 또는 배포에 대한 사용자의 명시적 승인을 받은 뒤에만 실행합니다. Git push가 필요하면 배포 승인과 별도로 push 승인을 받습니다.
...
사용자가 해당 Worker 배포를 명시적으로 승인한 경우에만 실행합니다.
...
`/교통정보` 명령어와 자동 알림(채널로 오는 것) 둘 다 도로별로 나누지
않고 한 메시지 안에 고속도로별로 묶은 텍스트로 보여줍니다 — 그대로
복사해서 다른 곳에 전달하기 좋습니다. 다만 디스코드 메시지 하나의 글자
수 제한(2000자)을 넘길 만큼 구간이 많으면(정체가 몰리는 시간대, 기능을
처음 켰을 때 등) 여러 메시지로 나눠 전부 보냅니다 — 뒤쪽 구간이 조용히
누락되지 않습니다.
