...
- 현재 작업 브랜치: `claude/how-it-works-x3928d`
- 로컬 작업 경로: `C:\Users\ForYou\Documents\ChatGPT\디스코드\music-validation-clone`
...
4. VPS의 `/opt/honorary-bot/.env` 존재 여부만 확인하되 비밀값을 출력하지 않는다.
5. VPS의 `bot.db`를 배포 과정에서 덮어쓰지 않는지 확인한다.
6. 코드 수정 후 로컬 문법 검사, VPS 문법 검사, 서비스 재시작, 최근 로그 확인 순으로 검증한다.
7. 완료된 변경은 현재 브랜치에 커밋하고 GitHub로 푸시한다.
...
레벨 및 칭호 관리 명령은 관리자 전용으로 제한했다. 봇 소유자 Discord 사용자 ID `246819817896673281`은 서버 권한과 관계없이 사용할 수 있도록 예외 처리했다. 단, 일반 조회 명령까지 전부 관리자 전용인지 여부는 각 데코레이터를 다시 확인한다.
...
- 음악 채널 ID와 대시보드 메시지 ID를 `bot.db`의 `guild_settings`에 서버별로 영구 저장
- 업데이트·재시작 뒤 저장된 ID로 기존 대시보드를 직접 복구하므로 `/음악대시보드설정`을 다시 실행할 필요 없음
- 기존 설치의 DB에는 시작 시 `music_channel_id`, `music_message_id` 열을 자동 추가하며 데이터는 삭제하지 않음
- 저장된 메시지가 삭제됐을 때만 같은 채널의 기존 고정 대시보드를 다시 찾고, 없으면 관리자가 설정 명령을 한 번 실행해 새 메시지를 생성
...
- Discord 사용자 ID `246819817896673281`은 어느 서버·채널에서든 사용 가능
...
운영 디렉터리는 `/opt/honorary-bot`이다. 변경 파일을 반영한 다음:
...
현재 작업 브랜치를 유지하되, 운영 자동 배포가 다른 브랜치를 감시한다면 병합 또는 배포 브랜치 정책을 먼저 확인한다.

### 자동 배포 권장 방식

- GitHub Actions에서 승인된 브랜치 push 감지
...
14. 레벨·칭호 관리 명령은 관리자 전용으로 제한하고 소유자 ID 예외를 적용했다.
...
GitHub 저장소는 https://github.com/Kimmacaroni/myhjbb-.git 이고 현재 작업 브랜치는 claude/how-it-works-x3928d 입니다. 로컬이나 VPS에 기존 변경이 있으면 절대 덮어쓰지 마세요. 실제 봇은 Hostinger VPS의 /opt/honorary-bot에서 systemd 서비스 honorary-bot으로 실행됩니다.
...
현재 최우선 과제는 식단/교통정보 설정 복구와 설정 데이터 보존 확인입니다. 코드를 수정하기 전에 현 상태와 로그를 먼저 진단하고, 수정 후 문법 검사, 서비스 활성 상태, 최근 로그, Discord 명령 동기화를 확인한 뒤 GitHub에 커밋·푸시하세요. 완료/미완료/사용자 입력 필요 사항을 명확히 구분해 보고해 주세요.
```
