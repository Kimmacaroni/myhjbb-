...
- 기본 모델: `Qwen/Qwen3-TTS-12Hz-0.6B-CustomVoice`
- 기본 목소리: `sohee` (한국어에 가장 적합)
- 선택 가능: `sohee`, `vivian`, `serena`, `uncle_fu`, `dylan`, `eric`,
  `ryan`, `aiden`, `ono_anna`, `mms_korean`
...
```bash
python experimental/huggingface_korean_tts.py "안녕하세요, 명예회장봇입니다." --voice sohee --output test.wav
...
```python
path = await tts.synthesize_async(message, "/tmp/honorary-tts.wav", voice="sohee")
```
